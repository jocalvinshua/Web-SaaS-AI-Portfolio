...
const Sidebar = () => {
    ...
    const sidebarLinks = [
        ...
        { name: "Resume", path: "/resume", icon: overviewicon },
        { name: "Template", path: "/", icon: chaticon },
    ];

    return (
        <>
            <aside className="md:w-64 w-20 border-r border-gray-200 bg-white flex flex-col h-full transition-all duration-300">
                <div className="flex-1 pt-4">
                    {sidebarLinks.map((item, index) => (
                        <NavLink 
                            ...
                            key={index}
                            className={({ isActive }) => `
                                flex items-center py-3 px-6 gap-3 transition-all
                                ${isActive 
                                    ? "border-r-4 border-primary bg-primary/10 text-primary font-bold" 
